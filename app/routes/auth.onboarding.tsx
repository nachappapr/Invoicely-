...
export async function action({ request }: ActionFunctionArgs) {
  ...
  const verifiedEmail = await requireVerifiedEmail(request);
  ...
  const submission = await parseWithZod(formData, {
    schema: () =>
      SignUpSchema.superRefine(async (data, ctx) => {
        ...
        return { ...data, session, email: verifiedEmail };
      }),
    async: true,
  });
  ...
  const verificationSession = await verifySesssionStorage.getSession(
    request.headers.get("cookie")
  );
  ...
  return redirect(safeRedirect(redirectTo, END_POINTS.HOME), {
    headers: combineHeaders(
      ...
      {
        "Set-Cookie": await verifySesssionStorage.destroySession(
          verificationSession
        ),
      }
    ),
